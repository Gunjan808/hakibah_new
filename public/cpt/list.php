...
<?php empty($PostData->type) or breadcrumb_start($this->uri->segment(3), 'add/cpt/' . $this->uri->segment(3), 'post_list');
?>
						...
						if (is($PostData, 'json') and is($PostData->data, 'array')) :
							foreach ($PostData->data as $key => $value) : ?>
								<tr>
									...
									<td>
										<div class="mr-2 mx-auto w-50 rounded-lg">
											<?php if (is($value->post_image)) : ?>
												<img src="<?php is($value->post_image, 'show'); ?>" class="img-fluid shadow rounded-lg" style="border: 2px solid #d3d3d3; width:80px " alt="<?php is($value->title, 'show'); ?>">
											<?php else : ?>
												<p>Image Not Found</p>
											<?php endif; ?>
										</div>
									...
									<td class="text-center">
										<p class="align-self-center mb-0 admin-name">
											<?php if (strlen($value->short_description) >= 26) : ?>
												...
												<div class="collapse" style="word-break: break-all;" id="showmore_<?php is($value->id, 'show'); ?>">
													<?php echo ucfirst(substr($value->short_description, 26)) ?>
												</div>
											<?php endif; ?>
										</p>
									</td>
										...
										<td class="text-center">
											<ul class="table-controls">
												<?php if (user_can('post_edit')) : ?>
													<li>
														<a href="<?php echo SITE_URL . 'edit/post/'  . $this->uri->segment(3) . '/' . $value->id;  ?>" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit">
