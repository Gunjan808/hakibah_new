...
class Lead extends CI_Controller
{
	...
	public function list_associate_program()
	{
		flash_message(
			'dashboard/login',
			is_login(),
		...
		$users = $this->UsersModel->all(
			['conditions' => "`user_type` = 'SUBSCRIBER' AND `status` != '3' AND `status` = '17' ORDER BY `is_pinned` DESC, `id` DESC"]
		);
		empty($users) or is_array($users) and $userData = json_decode(json_encode([
			'type' => 'ASSOCIATE PROGRAM',
			'data' => $users
		...
		$this->load->view('template/header');
		$this->load->view('template/sidebar');
		$this->load->view('leads/associate_program', compact('userData'));
		$this->load->view('template/footer');
	}
	...
	public function delete_lead($lead_slug = null)
	{
		...
		$userSlug = $this->UsersModel->first(['status!=' => 3, 'slug' => $lead_slug]);
		if (!is($userSlug, 'array')) {
			$userSlug = $this->UsersModel->first(['status!=' => 3, 'id' => $lead_slug]);
			$user = $this->UsersModel->updateTable([
				'status' => '3',
			], ['id' => $lead_slug]);
		} else {
