...
if (!function_exists('is_user_login')) {
	...
	{
		return is($_SESSION['LOGIN']) and is($_SESSION['USER_ID']) and is($_SESSION['USER_EMAIL']) and is($_SESSION['USER_UNIVERSITY_ID']) and is($_SESSION['USER_ACADEMIC_YEAR']) and is($_SESSION['USER_TYPE']) and $_SESSION['USER_TYPE'] === 'SUBSCRIBER';
	}
}
...
if (!function_exists('is')) {
	 ...
	 * @param string $action ex. `show`, `showCapital`, `price`, `json`, `rating`, `array`, `date`, `datetime`, `timeago`, `fulltimeago`
	 * @return bool */
	function is(&$param = null, ?string $action = null)
	{
		...
		} elseif (!is_null($action) and $action === 'timeago') {
			return isset($param) and $param !== false and $param != '' and !empty($param) and print(_time_ago($param));
		} elseif (!is_null($action) and $action === 'fulltimeago') {
			return isset($param) and $param !== false and $param != '' and !empty($param) and print(_time_ago($param, true));
		}
		return false;
	};
}
...
if (!function_exists('exists')) {
	 ...
	 * @return string */
	function exists(&$param = null, ?string $action = null)
	{
		if (is($action) and is($param, $action))
			return $param;
		if (is($param))
			return $param;
		return '';
...
if (!function_exists('is_login')) {
	...
	function is_login()
	{
		return is($_SESSION['USER_ID']) and is($_SESSION['USER_NAME']) and is($_SESSION['USER_FULLNAME']) and is($_SESSION['USER_EMAIL']) and is($_SESSION['USER_MOBILE']) and is($_SESSION['LOGIN']);
	}
}
